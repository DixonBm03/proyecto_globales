  ...
  async function fetchAQ(lat, lon) {
    try {
      ...
      if (!cur || cur.us_aqi == null)
        throw new Error('Sin us_aqi en la respuesta');
      ...
      if (hourly?.time?.length && hourly.us_aqi?.length) {
        ...
        if (aqi6 != null || aqi20 != null) {
          const best =
            aqi6 == null ? aqi20 : aqi20 == null ? aqi6 : Math.min(aqi6, aqi20);
      ...
      <div className='map-side'>
        ...
        {!loading && !err && data && (
          <>
            ...
            {data.bestHourText && (
              <p>
                Mejor hora hoy: <b>{data.bestHourText}</b>
              </p>
            )}
        ...
        {!point && !loading && !err && (
          <p>Haz clic en el mapa para consultar un punto.</p>
        )}
      </div>
    </div>
  );
