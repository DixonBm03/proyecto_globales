...
// Env (opcional) – CRA
const ENV_SERVICE  = (process?.env?.REACT_APP_EMAILJS_SERVICE_ID  || "");
const ENV_TEMPLATE = (process?.env?.REACT_APP_EMAILJS_TEMPLATE_ID || "");
const ENV_PUBLIC   = (process?.env?.REACT_APP_EMAILJS_PUBLIC_KEY  || "");

// Permite configurar desde la UI (localStorage) sin .env
const LS_SERVICE  = typeof window !== "undefined" ? localStorage.getItem("emailjs_service")  : "";
const LS_TEMPLATE = typeof window !== "undefined" ? localStorage.getItem("emailjs_template") : "";
const LS_PUBLIC   = typeof window !== "undefined" ? localStorage.getItem("emailjs_public")   : "";
    ...
    return (
        <div className="card">
            ...
            <div className="slider-row">
                <input type="range" min="0" max="300" value={aqi}
            ...
            <div className="advice" style={{ borderColor: cat.color }}>
                ...
                <p>{cat.msg}</p>
                {aqi <= 50 && <p>Estado general: <b>actividad normal</b>.</p>}
                {aqi > 50 && aqi <= 100 && <p>Evita esfuerzos intensos prolongados.</p>}
                {aqi > 100 && aqi <= 150 && <p>Personas sensibles: usar mascarilla si es necesario.</p>}
                {aqi >= 151 && aqi <= 200 && <p><b>No recomendable salir</b>. Prefiere espacios cerrados.</p>}
                {aqi > 200 && <p><b>Peligroso</b>: quédate en casa, filtra el aire si puedes.</p>}
            </div>
        </div>
    );
}
